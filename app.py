    ...
    with st.form("today"):
        for i in range(18):
            with st.expander(f"{i+1}頭目", expanded=i<6):
                ...
                if name: horses.append({"馬名":name,"人気":pop,"オッズ":odds})
...
with tab2:
    ...
    with st.form("past"):
        ...
        if st.form_submit_button("登録"):
            new = pd.DataFrame([{"日付":str(race_date),"馬名":win_name,"人気":win_pop,"オッズ":win_odds,"着順":1}])
            global df_past
            df_past = pd.concat([df_past, new]).drop_duplicates()
            df_past.to_csv("data.csv", index=False)
            st.success(f"{win_name} {win_odds}倍を登録！ 現在{len(df_past)}レース")

with tab3:
    st.subheader("舜を賢くする")
    st.write(f"現在の学習データ：{len(df_past)}レース")
    if len(df_past) >= 10:
        if st.button("今すぐAI学習させる！！", type="primary"):
            ...
            st.success("舜が超進化完了！！ 次から予想が出るで！！")
            st.balloons()
    else:
        st.info("あと10レース以上登録したら学習ボタン出現")
